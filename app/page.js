  ...
  useEffect(() => {
    async function fetchProducts() {
      try {
        ...
        console.error('Error fetching top matcha products for landing page:', err);
  ...
  return (
    <main className="min-h-screen bg-white">
      ...
      <section className="p-8">
        <h2 className="text-2xl font-bold text-matcha mb-6">top matcha picks</h2>

        {loading && <p className="text-center text-gray-700">Loading top matcha products...</p>}
        {error && <p className="text-center text-red-600">{error}</p>}
        {!loading && !error && topMatchaProducts.length === 0 && (
          <p className="text-center text-gray-700">No top matcha products found at the moment.</p>
        )}

<div className="grid grid-cols-2 md:grid-cols-4 gap-6">
  {!loading && !error && topMatchaProducts.map((product) => (
    <Link
      key={product.id}
    ...
    >
      ...
      />
      <h3 className="text-lg font-semibold text-green-800 text-center">{product.name}</h3>
